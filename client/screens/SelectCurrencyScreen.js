...
class SelectCurrencyScreen extends React.Component {
  ...
  changeMainCurrency = newMainCurrency => {
    ...
    Object.values(this.props.state.entities.expenses).forEach(expense => {
      const newMainCurrencyPerEuro = this.state.currencies[newMainCurrency]
        .fxRatePerEuro;
      const expenseCurrencyPerEuro = this.state.currencies[expense.currency]
        .fxRatePerEuro;
      const fxRatePerNewMainCurrency =
        expenseCurrencyPerEuro / newMainCurrencyPerEuro;
      updatedExpenses[expense.id].inMainCurrency = +(
        updatedExpenses[expense.id].amount / fxRatePerNewMainCurrency
      ).toFixed(2);
  ...
  onPress = currency => {
    const isGlobalChange = this.props.navigation.getParam(
      "isGlobalChange",
      false
    );
    if (isGlobalChange) {
  ...
  renderCurrency = ({ item }) => {
    const mainCurrencyPerEuro = this.state.currencies[this.state.mainCurrency]
      .fxRatePerEuro;
      ...
      >
        ...
        <View style={styles.currency__right}>
          ...
          <Text style={styles.currency__rb}>
            1 {this.state.mainCurrency} = {fxRatePerMainCurrency.toPrecision(5)}{" "}
            {item.id}
          </Text>
        </View>
      </TouchableOpacity>
  ...
  render() {
    const currencies = Object.values(this.props.state.entities.currencies).sort(
